...
const BaseRoute = BaseProto.extend<BaseRouteInterface>({
  ...
  set(action: string, ...props: Array<any>) {
    ...
    for (var a = 0; a < nameRoutes.length; a++) {
      if(nameRoutes[a] == props[1]){
        console.log('------------------------------------------------------------------------')
        console.log('  You have duplicate route name "'+props[1]+'" check your route again!');
        console.log('------------------------------------------------------------------------')
        process.exit(1);
      }
    }
    nameRoutes.push(props[1]);
    props[2] = [
      this._middleware,
      ...props[2]
    ]
